...
from pypedeid.synthesis.client import OpenAICompatibleChatClient
...
class Settings(BaseSettings):
    ...
    #: Active label space (see :mod:`pypedeid.labels`). The default
    ...
    #: Active risk profile (see :mod:`pypedeid.risk`). The default
    ...
    #: Active surrogate strategy pack (see :mod:`pypedeid.pipes.surrogate.packs`).
    ...
    auth_disabled: bool = Field(
        ...
        description=(
            ...
            "PYPEDEID_ENVIRONMENT=production. Default False: prod refuses to start without keys."
        ),
    )
    ...
    allow_external_llm: bool = Field(
        ...
        description=(
            ...
            "OpenAI-compatible endpoint. Default False; set PYPEDEID_ALLOW_EXTERNAL_LLM=true "
            "to acknowledge that text passed to those features will leave the host."
        ),
    )
    #: For :class:`~pypedeid.synthesis.client.OpenAICompatibleChatClient`. Loaded from ``.env`` or the environment. Either ``OPENAI_API_KEY`` or ``PYPEDEID_OPENAI_API_KEY`` may be set.
    openai_api_key: str | None = Field(
        default=None,
        validation_alias=AliasChoices("OPENAI_API_KEY", "PYPEDEID_OPENAI_API_KEY"),
    )
    openai_base_url: str | None = Field(
        default=None,
        validation_alias=AliasChoices("OPENAI_BASE_URL", "PYPEDEID_OPENAI_BASE_URL"),
    )
    openai_model: str = Field(
        default="gpt-4o-mini",
        validation_alias=AliasChoices("OPENAI_MODEL", "PYPEDEID_OPENAI_MODEL"),
    )
    #: Base URL for the NeuroNER Docker HTTP sidecar (overridable per pipe via ``base_url`` on :class:`~pypedeid.pipes.neuroner_ner.pipe.NeuroNerConfig`).
    neuroner_http_url: str = Field(
        default="http://127.0.0.1:8765",
        validation_alias=AliasChoices(
            "PYPEDEID_NEURONER_HTTP_URL",
            "NEURONER_HTTP_URL",
        ),
    )

    model_config = SettingsConfigDict(
        env_prefix="PYPEDEID_",
    ...
    def _legacy_processed_dir_env(self) -> Self:
        """``PYPEDEID_PROCESSED_DIR`` was the old name for the corpus data root; still honored."""
        if os.environ.get("PYPEDEID_PROCESSED_DIR") and not os.environ.get(
            "PYPEDEID_CORPORA_DIR"
        ):
            logger.warning(
                "PYPEDEID_PROCESSED_DIR is deprecated; use PYPEDEID_CORPORA_DIR "
                "(single directory for all corpus files and API materialized outputs)."
            )
            self.corpora_dir = Path(os.environ["PYPEDEID_PROCESSED_DIR"])
        return self
    ...
    def require_external_llm_allowed(self) -> None:
        ...
        if not self.allow_external_llm:
            raise ValueError(
                "External LLM calls are disabled. Set PYPEDEID_ALLOW_EXTERNAL_LLM=true "
    ...
    def openai_chat_client(self) -> OpenAICompatibleChatClient:
        ...
        if not self.openai_api_key:
            raise ValueError(
                "OpenAI API key is not set. Add OPENAI_API_KEY (or PYPEDEID_OPENAI_API_KEY) "
