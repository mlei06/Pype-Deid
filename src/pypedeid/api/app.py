...
from pypedeid.api.routers import audit, datasets, deploy, dictionaries, evaluation, inference, models, pipelines, process
from pypedeid.api.schemas import HealthResponse
from pypedeid.db import get_engine, init_db

logger = logging.getLogger("pypedeid")


@asynccontextmanager
async def lifespan(_app: FastAPI):
    init_db()
    from pypedeid.api.auth import auth_enabled
    ...
    if not auth_enabled() and get_settings().environment == "production":
        if not get_settings().auth_disabled:
            raise RuntimeError(
                "API auth is DISABLED and PYPEDEID_ENVIRONMENT=production. "
                "Refusing to start. Either configure PYPEDEID_ADMIN_API_KEYS / "
                "PYPEDEID_INFERENCE_API_KEYS, or set PYPEDEID_AUTH_DISABLED=true "
                "to acknowledge an intentionally open deployment."
            )
        logger.warning(
            "API auth is DISABLED in production posture (PYPEDEID_AUTH_DISABLED=true). "
...
def create_app() -> FastAPI:
    ...
    from pypedeid.api.auth import auth_enabled, get_api_key_scope_for_health, require_admin
    from pypedeid.config import get_settings
    ...
    # CORS — configured via settings (env var PYPEDEID_CORS_ORIGINS or .env).
