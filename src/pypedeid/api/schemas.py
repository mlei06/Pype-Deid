...
class HealthResponse(BaseModel):
    ...
    label_space_name: str = Field(
        description="Active LabelSpace (``PYPEDEID_LABEL_SPACE_NAME``) — used for "
        "``POST /process/*`` span label normalization, not for evaluation."
    )
    risk_profile_name: str = Field(
        description="Default risk profile (``PYPEDEID_RISK_PROFILE_NAME``) for eval when not overridden."
    )
