...
```
data/
  pipelines/            Named pipeline JSON configs  (PYPEDEID_PIPELINES_DIR)
                        — seed files are tracked; operator edits via Playground
                          admin UI or on disk
  modes.json            Deploy config: mode aliases, allowlist, production URL
                        (PYPEDEID_MODES_PATH; mutable via PUT /deploy)
  evaluations/          Eval result JSON            (PYPEDEID_EVALUATIONS_DIR)
  inference_runs/       Saved batch inference runs  (PYPEDEID_INFERENCE_RUNS_DIR)
  app.sqlite            Audit log (SQLite)          (PYPEDEID_DATABASE_URL)
  corpora/              Dataset homes (JSONL-only)  (PYPEDEID_CORPORA_DIR)
  ...
  exports/              Materialized exports        (PYPEDEID_EXPORTS_DIR)
  ...
  dictionaries/         Whitelist / blacklist term lists (PYPEDEID_DICTIONARIES_DIR)
...
| `evaluations/` | One JSON file per eval run (Playground, API, or **CLI**); the Evaluate view lists `*.json` for history. | `pypedeid eval`, `POST /eval/run`, `GET /eval/runs` |
| `inference_runs/` | Batch inference snapshots | `pypedeid batch`, `POST /process/*` |
| `app.sqlite` | Audit log | Written by `log_run()` on every run |
| `corpora/` | Canonical **JSONL** datasets | `pypedeid dataset register` (JSONL), `import-brat`, `POST /datasets/*` — **tracked** examples: `sample_notes` and `sample_notes_surrogated` (15 docs each) |
...
| `exports/` | Exports (BRAT, training formats) | `POST /datasets/{name}/export`, `pypedeid dataset export` |
| `dictionaries/` | Whitelist / blacklist term lists | `pypedeid dict import`, `POST /dictionaries` |
